...
const fs = require('fs');
const myip = '47.237.186.166' //换成你的服务器IP
function rp(url)
{
...
{
	return Promise.all([PROXY_INFO_ADDR,PROXY_SECRET_ADDR].map(function(url)
	{
		return rp(url)
	})).then(function(arr)
	{
		...
		my_ip = Buffer.from(myip.toString().split('.'));
		fillPool();
...
async function handleClient(client,filter,id)
{
	...
	if (h1.compare(TLS_START_BYTES,0,11,0,11)===0)
	{
		...
		let handshake_hex = handshake.toString('hex');
		// 查找 .t.cn 的 hex 表达式
		const sniHex = '2e742e636e';
		const idx = handshake_hex.indexOf(sniHex);
		let sniHexStr = '';
		if (idx !== -1 && idx >= 64) {
			// 前32位hex（16字节）
			sniHexStr = handshake_hex.substring(idx - 32, idx);
		} else {
			// SNI 未找到，拒绝连接
			client.destroy();
			return;
		}
	const SNI = Buffer.from(sniHexStr, 'hex').toString();
	sniForLog = SNI;
	...
	client.once('finished',function(err)
	{
		...
		try {
			if (sniForLog) {
				const logLine = `${new Date().toISOString()} SNI: ${sniForLog} bytesRead: ${client.bytesRead} bytesWritten: ${client.bytesWritten}\n`;
